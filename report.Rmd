...
Next, I conduct a RFM Analysis to segment customers based on how recently a customer made a purchase (recency), how often a customer makes a purchase (frequency), and how much money a customer has spent on purchases (monetary value). Table 1 lists the features we consider in this analysis. The variable "latest_visit_date" is computed via subtracting the values of the variable recency (which is the number of days since the customer's last purchase) from the date 2014/10/04. The values of the variables "latest_visit_date", "num_of_purchases", "expenditure" represent recency, frequency, and monetary value, respectively.

```{r, include=FALSE}
meta = data.frame(c("id: the customer's unique identifier", "latest_visit_date: the date of the customer's latest visit", "num_of_purchases: the total number of purchases made in stores, through the company's website and using a catalogue", "expenditure:the total amount of money spent on fruits, meat, fish, sweet and gold")) %>% as_tibble()
```


```{r}
kable(meta, caption = 'List of variables used', col.names = "Variable")
```
...
I sum the amount of money spent on each of fruits, meat, fish, sweet and gold (the total amount is referred as expenditure), and examine how customers' expenditure varies by their education level, income, and marital status. 

![](figures/figure2.png)

In Figure 2, we can see that customers' expenditure roughly increases as their income increases, but there are some outliers at high income values. The expenditure of customers with undergraduate degree seems to be slightly lower than that of customers with postgraduate degree. Also, customers without any child have much higher expenditure than those who have childs, but customers' expenditure does not differ much by their marital status.  


I select the following variables to predict expenditure. 

```{r}
res <- readRDS(file = 'derived_data/res_models.rds')

kable(res, caption = 'Model Prediction Accuracy Comparison')
```

# Future Direction

Here we only analyze the total expenditure. 
